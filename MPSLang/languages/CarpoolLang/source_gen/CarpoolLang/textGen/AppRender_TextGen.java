...
public class AppRender_TextGen extends TextGenDescriptorBase {
  ...
  public void generateText(final TextGenContext ctx) {
    ...
    if (SPropertyOperations.hasValue(SLinkOperations.getTarget(ctx.getPrimaryInput(), LINKS.componentName$1j5l), PROPS.componentName$i_FD, "Profile")) {
      ...
      tgs.append("</div>\n");
      tgs.indent();
      tgs.append("<Tabs defaultValue=\"profile\" className=\"space-y-6\">\n");
      ctx.getBuffer().area().increaseIndent();
      tgs.indent();
      tgs.append("<TabsList>\n");
      ctx.getBuffer().area().increaseIndent();
      tgs.indent();
      tgs.append("<TabsTrigger value=\"profile\">Profile</TabsTrigger>\n");
      tgs.appendNode(ListSequence.fromList(SLinkOperations.getChildren(ctx.getPrimaryInput(), LINKS.listOfChildRenders$9qWf)).last());
      ctx.getBuffer().area().decreaseIndent();
      ctx.getBuffer().area().decreaseIndent();
      tgs.indent();
      tgs.append("</Tabs>\n");
